...
app.post('/upload', upload.single('clinicalNote'), async (req, res) => {
  try {
    const patientData = JSON.parse(req.body.patientData);     const clinicalNote = fs.readFileSync(req.file.path, 'utf8'); 
...
function createFhirBundleUsingFhirJs(patientData, clinicalNote) {
  return {
    ...
    type: 'transaction',     entry: [
      {
        fullUrl: "urn:uuid:" + generateUUID(),         resource: {
...
app.get('/api/patient/:name', async (req, res) => {
  ...
  let patientData = await getPatientDataFromNeo4j(patientName); 
  ...
  patientData.age = convertNeo4jInt(patientData.age);

  const mockedData = getMockedData(patientName); 
